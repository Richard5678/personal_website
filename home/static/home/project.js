...
var algo_content = {
    ...
    "gomoku": {
        "en": {
            "content": ["This project uses Monte Carlo Tree Search (MCTS) to construct ai AI for gomoku on a 19 by 19 board. "
                        + "For board games, we can imagine the human player standing at the root of the tree, its children nodes should be all possible "
                        + "moves that the next player (AI) can make. We expand the tree recursively until we are at a terminal state (someone has won) "
                        + "The problem with this approch is that the tree grows exponentially which means we simply cannot explore all possibilities on a 19 by 19 board. <br>"
                        + "Two popular tree search algorithms address this problem: "
                        + "<ol>"
                        + "     <li>Minimax: <br>"
                        + "     Minimax differs from the brute-force tree search algorithm from its value function and max-depth. "
                        + "     When the search tree we have contructed reaches the max-depth, we will use the value function to evalute the state of the board at leaf nodes. "
                        + "     Then the evaluation result is backpropagated all the way up to the root of the tree by alternating bewteen choosing the max or min of all child nodes. "
                        + "     For example, suppose the max-depth is 3, we are standing at the root node and are ready to make to move down the tree:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku1.png\">"
                        + "     </div>"
                        + "     Suppose the higher the output the value network, the better the ai player is at. Then we would prefer the children that has the max value from the root node. "
                        + "     However, when we move down the tree to depth 1, it is the other player's turn to make a move and their goal is to minimize the board value for the ai player, "
                        + "     so we will choose the child with the minimum value of its children nodes at depth 1. The process of choosing the max/min nodes alters until we have reached the leaf node. "
                        + "     Evantually, we will choose the move that the right child node represents at the root node (4 > 3). <br>"
                        + "     <div style=\"margin-top:20px\"></div>"
                        + "     One problem the minimax algorithm suffers is that we need a value function and such function is usually hard to find. That leads the existance of MCTS. "
                        + "</li>"
                        + "     <li style=\"margin-top:50px;\">Monte Carlo Tree Search (MCTS): <br>"
                        + "     MCTS can be divided into four steps below"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku2.png\">"
                        + "     </div>"
                        + "     During the selection phase we will move toward the child node with the highest ucb value which is calculated as:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku4.png\">"
                        + "     </div>"
                        + "     where the mean node value is the number of times ai won divided by the number of times we have visited the node.  <br>"
                        + "     To be more specific, MCTS follow the flowchart below:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku5.png\">"
                        + "     </div>"
                        + "     The simulation phase (rollout) randomly places stones on the board until someone won. <br>"
                        + "     An concrete example we update value is give below:"
                        + "     <div style=\"text-align: center; background-color: white\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku3.png\">"
                        + "     </div>"
                        + "     After we have repeated this process enough iteratioins, then we will choose the move represented by the child node of the root which is visited the most (highest denominator). "
                        + "     <p style=\"margin-top: 40px;\">MCTS does not need a value function which solves the problem imposed by minimax. It has also been proven that MCTS converges to the optimal solution as the number of iterations increases. </p>"
                        + "     </li>"
                        + "</ol>"
                        + "     <h3 style=\"margin-top:50px;\">Obstacles During Implementations</h3>"
                        + "     The main problem I had was that I found that MCTS does not perform well on a 19 by 19 board if we consider all possible moves in the expansion phase. "
                        + "     I think this is because with limited computational power we are unable to boost the confidence in our predictions. "
                        + "     For example, if we run MCTS for 50000 iterations and there are 360 possible moves white could do after black's first move, then on average each possible move will only be simulated just over 100 times which is far from enough. "
                        + "     Based on the rules of gomoku, I made the assumption that the best move will always be around existing stones on the board. "
                        + "     Therefore, I defined all possible actions in the expansion phase all moves that shares the same block with an existing stone. "
                        + "     This assumption greatly improved the predictions made by MCTS without losing much generality. <br>"
                        + "     <div style=\"margin-top:30px;\"></div>"
                        + "     Another obstable I faced was there seems to be too much randomness in the simulation stage and the AI seems to be more focusing on itself. "
                        + "     For example, suppose black has a live 3 while white only has a live 2, which means white needs to take action to block black's live 3. "
                        + "     However, white almost always will place a stone which forms live 3 of itself instead of blocking black. In fact, the number of visits (what we use to choose moves) differs greatly bewteen blocking black and forming a white's live 3. "
                        + "     Intuitively, forming a live 3 is probably the second best move in a human player's eye. However, blocking black's live 3 is obviously more urgent. "
                        + "     It does not seem to be a problem caused by the lack of number of iteration, but rather caused by the randomness introduced in the rollout. "
                        + "     I read the original <a href=\"https://storage.googleapis.com/deepmind-media/alphago/AlphaGoNaturePaper.pdf\">AlphaGo paper</a> and it trains fast policy network during rollout. (Page 1 mid-right). "
                        + "     If I am not interpreting this wrong, then the rollout phase is not purely random, instead, it uses a policy network which is able to predict the next move. "
                        + "     This approch greatly limits the randomness involved in the rollout. <br>"
                        + "     <div style=\"margin-top:40px\"></div>To prevent this scenerio from happeing I have implemented functions to check for live 3 and consecutive 4 to make sure appropriate moves are chosen. ", 

                        "       <div style=\"text-align: center; background-color: white; width: 90%\">"
                        + "         <img style=\"width: 90%;background-color: white;\" src=\"/static/home/gomoku6.png\">"
                        + "     </div>"
                        + "     A blue quarter circle is placed on the last move. A bar on the right displays the winning percentage for both players which is obtained from simulations. "
                        + "     The reset button will reset the board to initial set when clicked. Undo button would remove the last move made by the human as well as the ai move following that. "],
            "implementation": ["See <a href=\"https://github.com/Richard5678/gomoku-ai\">github</a> for details."]
        },
        "zh": {
            "content": ["这个项目使用了蒙特卡洛树搜索(Monte Carlo Tree Search)构建了在19乘19棋盘上下五子棋的人工智能。"
                        + "在棋盘游戏中，我们可以想象人类棋手站在决策树的根节点，他的子节点为所有人工智能在下一步可能选择的步骤。"
                        + "我们通过递归来延展树知道遇到终端状态（一个棋手赢得了对局）。"
                        + "这个方法的问题是决策树的节点会呈指数增长，这意味着列举出19乘19棋盘上所有可能是不能够实现的。<br>"
                        + "两个解决这个问题并且受欢迎的树搜索算法为: "
                        + "<ol>"
                        + "     <li>极小化极大算法（Minimax）: <br>"
                        + "     极小化极大与以上的蛮力算法不同的是他有一个价值方程和一个最高深度。"
                        + "     当搜索树被建造到最高深度时，我们用价值方程来衡量叶节点棋盘的状态。"
                        + "     之后测量结果会反向传播(backpropagate)直到树的跟节点(过程中每个节点会交替取其所有子节点最大值或最小值)。"
                        + "     比如，假设我们的最高深度为3，我们站在跟节点并且准备沿着树向下走一步（到其一个子节点）:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku1.png\">"
                        + "     </div>"
                        + "     加入棋盘状态对于人工智能越好，价值方程的输出就越高。之后我们会选择所有子节点中价值最高的那个。"
                        + "     然而，当我们向下移动到深度1时，这时是对手的回合并且它们希望人工智能的价值数值越小越好, "
                        + "     所以这时我们会选择深度1的节点的子节点中价值最小的节点。我们将交替选择最大货最小子节点知道叶节点。"
                        + "     最终我们会选择跟节点右边的子节点代表的步骤(4 > 3)。<br>"
                        + "     <div style=\"margin-top:20px\"></div>"
                        + "     极小化极大算法的一大问题是我们需要定义价值方程然而这样的方程在实际应用中很难找到。这个问引出了蒙特卡洛树搜索的存在。"
                        + "</li>"
                        + "     <li style=\"margin-top:50px;\">蒙特卡洛树搜索 (MCTS): <br>"
                        + "     MCTS 可以被分为以下四个步骤:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku2.png\">"
                        + "     </div>"
                        + "     在selection阶段我们会朝有最高ucb值的子节点走一步。ucb为:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku4.png\">"
                        + "     </div>"
                        + "     节点平均值(mean node value)是在模拟中AI赢得的对局数处以总模拟对局数。<br>"
                        + "     更具体地说，MCTS是根据以下的流程图实行的:"
                        + "     <div style=\"text-align: center;\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku5.png\">"
                        + "     </div>"
                        + "     模拟阶段(rollout)随机将棋子放在棋盘上知道一个棋手赢得了对局。<br>"
                        + "     以下为一个如何更新节点数值的实例:"
                        + "     <div style=\"text-align: center; background-color: white\">"
                        + "         <img style=\"width: 70%;\" src=\"/static/home/gomoku3.png\">"
                        + "     </div>"
                        + "     在循环这个过程足够多次数后, 我们将跟节点的子节点中被经过次数最多(最高的分母）的节点代表的步骤作为AI的下一步。"
                        + "     <p style=\"margin-top: 40px;\">MCTS不需要一个价值方程，这解决了minimax介绍的问题。并且MCTS已经被证明会随着循环次数的增多而收敛到最佳答案。</p>"
                        + "     </li>"
                        + "</ol>"
                        + "     <h3 style=\"margin-top:50px;\">软件实现中的困难</h3>"
                        + "     我遇到的主要的问题是我发现MCTS在19乘19的棋盘上表现的很不好如果我们在延展阶段(expansion phase)考虑棋盘上所有可能的下一步。"
                        + "     我认为这主要是因为有限的计算能力导致了我们无法提高对我们做出的预测的信心。"
                        + "     根据五子棋的规则, 我做的以下假设:最佳的下一步总是在棋盘上以后棋子的周围。"
                        + "     所以，我定义在延展阶段的所有可能的下一步为所有与已有棋子在同一个方格上的交叉点。"
                        + "     这个假设极大的提升了MCTS做的预测并且没有丢失算法的广泛性。<br>"
                        + "     <div style=\"margin-top:30px;\"></div>"
                        + "     另一个难题是模拟阶段看起来有太多的随机性并且AI似乎更专注与自身的发展。"
                        + "     比如，加入黑棋有一个活三而白棋最多只有一个活二，这意味着白棋需要在下一步挡住黑棋活三的一头。"
                        + "     然而，白棋似乎总是会在下一步将子集的活二变成活三而不是阻挡黑棋的活三。事实上，与阻挡黑棋的下一步相比，白棋形成活三的步骤的节点被经过的次数(其决定我们下一步的选择)要高很多。"
                        + "     直观上讲白棋形成一个活三应该是人类眼中第二号的步骤。然而，阻挡黑棋的活三显然更紧急。"
                        + "     这个问题看起来不像是由于循环次数不够多导致的，其更像是在模拟阶段的随机性导致的。（随机性导致算法不能很好的分辨不同的下一步的好坏）。"
                        + "     我阅读了最初的<a href=\"https://storage.googleapis.com/deepmind-media/alphago/AlphaGoNaturePaper.pdf\">AlphaGo 论文</a>，其在模拟阶段钱训练一个快速决策网(fast policy network)。(第一页右中)。"
                        + "     如果我没有理解错的话，模拟阶段不是纯随机的，取而代之的是其运用了一个能够预测下一步的决策网络。"
                        + "     这个方法很大程度的减少了模拟阶段的随机性。<br>"
                        + "     <div style=\"margin-top:40px\"></div>为了组织以上的情况发生我包含了一些发现活三冲四的函数来确保正确的步骤在模拟阶段被选取。", 

                        "       <div style=\"text-align: center; background-color: white; width: 90%\">"
                        + "         <img style=\"width: 90%;background-color: white;\" src=\"/static/home/gomoku6.png\">"
                        + "     </div>"
                        + "     最后一步下的棋子上会有一个蓝色的四分之一园。右边有一个进度条来通过模拟计算出的双方的胜率。"
                        + "     reset按键会在点击时把棋盘设置成初始状态。Undo按键会移除人类棋手所下的上一步以及人工智能在之后一步。"],
            "implementation": ["详情见<a href=\"https://github.com/Richard5678/gomoku-ai\">github</a>。"]
        },
        "img": "/static/home/gomoku.png"
    }
}